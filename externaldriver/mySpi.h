...
struct spi_ops
{
    ...
    int (*read)(spi_device_t device, uint8_t *buf, uint32_t len, SPI_PCS_Type cs_return_inactive);
    int (*write)(spi_device_t device, uint8_t *buf, uint32_t len, SPI_PCS_Type cs_return_inactive);
};
...
static inline int spi_read(spi_device_t device, uint8_t *buf, uint32_t len, SPI_PCS_Type cs_return_inactive)
{
    uint32_t ret;
...
static inline int spi_write(spi_device_t device, uint8_t *buf, uint32_t len, SPI_PCS_Type cs_return_inactive)
{
    uint32_t ret;
    if(device != device->bus->owner)
    {
