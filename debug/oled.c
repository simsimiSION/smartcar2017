...
//将这个改成置高或者置低的函数 用来替代原来的赋值为高或者低的地方
#define OLCD_SCL_LOW    OLED_DO_OUT_LOW
#define OLCD_SCL_HIGH   OLED_DO_OUT_HIGH

#define OLCD_SDA_LOW    OLED_DI_OUT_LOW
#define OLCD_SDA_HIGH   OLED_DI_OUT_HIGH

#define OLCD_RST_LOW    OLED_RST_OUT_LOW
#define OLCD_RST_HIGH   OLED_RST_OUT_HIGH

#define OLCD_DC_LOW     OLED_DC_OUT_LOW
#define OLCD_DC_HIGH    OLED_DC_OUT_HIGH

#define OLCD_CS_LOW     OLED_CS_OUT_LOW
#define OLCD_CS_HIGH    OLED_CS_OUT_HIGH
...
//uint8_t _coordinate_x = 121, _coordinate_y = 7;      //在反向显示时用
uint8_t _coordinate_x = 0, _coordinate_y = 0;
...
//{ 0x80, 0x90, 0x90, 0x90, 0xFE, 0x00 },
//{ 0x5E, 0x92, 0x92, 0x82, 0x7C, 0x00 },
...
//{ 0x3E, 0x25, 0x25, 0x25, 0x18, 0x00 },
//{ 0x1E, 0x20, 0x20, 0x10, 0xFE, 0x00 },
//{ 0x00, 0x02, 0xBE, 0x22, 0x00, 0x00 },
//{ 0x00, 0xBE, 0x21, 0x01, 0x02, 0x00 },
//{ 0x00, 0x22, 0x14, 0x08, 0xFE, 0x00 },
...
void LCD_WrDat(u8 data)
{
	...
	//LCD_CS=0;;
        OLCD_CS_LOW;
	OLCD_DC_HIGH;//OLCD_DC=1;
        OLCD_SCL_LOW;//OLCD_SCL=0;
  //asm("nop");
  while(i--)
  {
    if(data&0x80){OLCD_SDA_HIGH;}
    else{OLCD_SDA_LOW;}
    OLCD_SCL_HIGH;
    asm("nop");;
    OLCD_SCL_LOW;;
...
void LCD_Init(void)
{
  GPIO_QuickInit(HW_GPIOB,9, kGPIO_Mode_OPP);///DO
  GPIO_SetBit(HW_GPIOB, 9);
  GPIO_QuickInit(HW_GPIOB, 11, kGPIO_Mode_OPP);///DI
  GPIO_SetBit(HW_GPIOB, 11);
  GPIO_QuickInit(HW_GPIOB, 17, kGPIO_Mode_OPP);///RST
  GPIO_SetBit(HW_GPIOB, 17);
  GPIO_QuickInit(HW_GPIOB, 21, kGPIO_Mode_OPP);///DC
  GPIO_SetBit(HW_GPIOB, 21);
  GPIO_QuickInit(HW_GPIOB, 23, kGPIO_Mode_OPP);///CS
...
/*========================================================================
* 函数名称：LCD_write_float
* 函数功能：显示float
* 备注：num-输入的浮点数 sig-保留的小数位数
========================================================================*/
void LCD_write_float( float num, uint8_t sig)
{
    ...
    i++;
    while(i--)
		LCD_write_char(n[i]+'0');

	LCD_write_char('.');			//输出小数点

	for(i=0;i<sig;i++)			//输出小数部分
	{
		fr*=10;
		LCD_write_char((int)fr+'0');
		fr-=(int)fr;
	}
}
...
void LCD_write_char(u8 ch){
  u8 c=0,i=0;
  c =ch-32;
        LCD_Set_Pos(_coordinate_x,_coordinate_y);
  	for(i=0;i<6;i++){
  	    LCD_WrDat(F6x8[c][i]);
        }
        
        _coordinate_x+=6;
        if(_coordinate_x >= 122){
            _coordinate_y++;
            _coordinate_x=0;
        }
...
void LCD_write_int(int num)
{
    ...
    //也可以用以下代码
	/*    for(; i > 0; i--)
	LCD_write_char(n[i]+'0');	//转化成ASCII并且输出;
    LCD_write_char(n[i]+'0');	*/
...
//参数：x的范围0～116，y为页的范围0～7
//返回：无
//==============================================================
void LCD_set_XY(u8 x, u8 y){
//    _coordinate_x = 121 - x;
//    _coordinate_y = 7 - y;         这两行在反向显示时用
